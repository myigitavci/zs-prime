...
from tensorflow.keras.layers import Layer, Conv2D, Activation, BatchNormalization, \
    Add, Conv2DTranspose, LeakyReLU, Lambda, ReLU
...
def conv2D_bn_nonlinear2(x, num_out_chan, kernel_size, activation_type='relu', USE_BN=True, layer_name='', strides = (1, 1), alpha = 0.3 ):
    with K.name_scope(layer_name):
        x = Conv2D(num_out_chan, kernel_size, activation=None, padding='same', kernel_initializer='truncated_normal', strides=strides)(x)
        if USE_BN:
            x = BatchNormalization()(x)
        ...
        if activation_type == 'LeakyReLU':
            return LeakyReLU(alpha=alpha)(x)
        if activation_type == 'NL':
            return x+ReLU()((x-1)/2)+ReLU()((-x-1)/2)
        else:
            return Activation(activation_type)(x)
def RegConvLayers2(nx,ny,ne,nLayers,num_filters):
    ...
    rg_term = conv2D_bn_nonlinear2(input_x, num_filters, filter_size, activation_type=AT, USE_BN=False, layer_name='')
    first_layer=rg_term
    for lyr in range(0,nLayers):
        previous_layer=rg_term
        rg_term = conv2D_bn_nonlinear2(rg_term, num_filters, filter_size, activation_type=AT, USE_BN=False, layer_name='')
        rg_term = conv2D_bn_nonlinear2(rg_term, num_filters, filter_size, activation_type='linear', USE_BN=True, layer_name='')
       # rg_term=rg_term*10
        rg_term = Add()([rg_term,previous_layer])
    rg_term = conv2D_bn_nonlinear2(rg_term, num_filters, filter_size, activation_type='linear', USE_BN=False, layer_name='')
    rg_term = Add()([rg_term,first_layer])

    # go to image space
    rg_term = conv2D_bn_nonlinear2(rg_term, 2*ne, (3,3), activation_type='linear', USE_BN=False, layer_name='')
