...
class myDC(Layer):
    ...
    def call(self, x):
        ...
        inp =  (tf.cast(csm, dtype=tf.complex64), tf.cast(mask, dtype=tf.complex64), tf.cast(field, dtype=tf.complex64), tf.cast(rhs, dtype=tf.complex64))
...
def create_modl2(nx, ny, nc, nLayers, num_block, num_filters = 64,  esp = 0):

    # define the inputs
    print('defining network input')
    ...
    # define variable
    print('defining network variable')
    ...
    # loop
    print('defining network loop')
    for blk in range(0,num_block):
        # concat shots with VC
        print('defining network DC')
