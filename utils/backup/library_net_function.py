...
class tfft2(Layer):
    ...
    def call(self, x):
        ...
        kt = tf.signal.fft2d(xt) # / tf.math.sqrt(tf.cast(nx*ny,tf.complex64))
...
class tifft2(Layer):
    ...
    def call(self, x):
        ...
        it = tf.signal.ifft2d(it) # * tf.math.sqrt(tf.cast(nx*ny,tf.complex64))
...
# def ConvLayers_real(nx,ny,ne,nLayers,num_filters):
...
#     # go to image space
#     rg_term     = conv2D_bn_nonlinear(rg_term, ne, (1,1), activation_type='linear', USE_BN=False, layer_name='')
