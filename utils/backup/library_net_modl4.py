...
class myDC(Layer):
    ...
    def __init__(self, **kwargs):
        ...
        self.lam2 = self.add_weight(name='lam2', shape=(1,), initializer=tf.constant_initializer(value=0.03),
                                     dtype='float32', trainable=True)        
        self.field = self.add_weight(name='b0map', shape=(220,220), initializer=tf.constant_initializer(value=0.0),
                                     dtype='float32', trainable=True)
    ...
    def call(self, x):
        rhs, csm, mask, fmt, timeline, bool_pol = x
        lam3 = tf.complex(self.lam1 + self.lam2, 0.)

        def fn1(tmp1):
            c1, m1, r1 = tmp1  
            # [c,x,y] -> [x,y,c]
            in_tmp = (tf.transpose(c1,perm=[1,2,0]),m1,self.field,r1)
...
def create_modl4(nx, ny, nc, nLayers, num_block, num_filters = 64,  esp = 0, unet_filters=8, unet_depth=4):
    ...
    for blk_outer in range(0,4):                
        ...
        for blk in range(0,4): #range(0,num_block):          
            ...
            rg2         = Add()([Atb2, rg2])         
            # Update DC
            dc1         = UpdateDC([rg1, input_c, input_m1, fftmtx, timeline, tf.constant(True)])      
            dc2         = UpdateDC([rg2, input_c, input_m2, fftmtx, timeline, tf.constant(False)])  
