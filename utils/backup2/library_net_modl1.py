...
def create_modl1(nx, ny, nc, nLayers, num_block, num_filters = 64):
    ...
    for blk in range(0,num_block):          
        ...
        dc_cat_i    = Concatenate(axis=-1)([dc1,tf.math.conj(dc1)])
        dc_cat_k    = Concatenate(axis=-1)([myFFT([dc1]),myFFT([tf.math.conj(dc1)])])             
        # CNN Regularization
        rg_term_i   = RegConv_i(dc_cat_i)
        rg_term_k   = RegConv_k(dc_cat_k)    
        # separate shots        
        irg1        = (rg_term_i[:,:,:,0:2] + tf.math.conj(rg_term_i[:,:,:,2:4]))/2
        krg1        = (myIFFT([rg_term_k[:,:,:,0:2]]) + tf.math.conj(myIFFT([rg_term_k[:,:,:,2:4]])))/2
    ...
    return Model(inputs     =   [ input_c, input_m1, input_k1],
                 outputs    =   [ out1 ],
                 name       =   'MODL1' )
