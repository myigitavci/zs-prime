...
def mosaic(img, num_row, num_col, fig_num, clim, path_fig_save,title='' ,use_transpose=False, use_flipud=False):
    ...
    plt.savefig(path_fig_save+title+'.png')
...
def uniform_selection1d(input_data, input_mask, rho, c_sq = 5):
  nrow, ncol = input_data.shape[0], input_data.shape[1]
  
  center_kx = int(find_center_ind(input_data, axes=(1, 2)))
  center_ky = int(find_center_ind(input_data, axes=(0,2)))
  temp_mask = np.copy(input_mask)
  temp_mask[:, center_ky] = 0
  temp_mask[:, center_ky - c_sq] = 0
  temp_mask[:, center_ky + c_sq] = 0
  #temp_mask[:, center_ky - c_sq // 2: center_ky + c_sq // 2 + 1] = 0
  pr = temp_mask[0, :]

  ind = np.random.choice(np.arange(nrow), size=int(np.count_nonzero(pr) * rho), replace=False, p=pr/np.sum(pr))
