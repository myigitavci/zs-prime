...
class Aclass:
    ...
    def myAtA(self,img):
        with tf.name_scope('AtA2'):
            ...
            # coilImgs : [y,c]
            coilImgs        =       tf.linalg.matmul( self.fmt/220, temp, adjoint_a=True )
...
def myCG(A,rhs):
    rhs=r2c(rhs)
    cond=lambda i,rTr,*_: tf.logical_and( tf.less(i,10), rTr>1e-7)
...
class myDC(Layer):
    ...
    def call(self, x):
        ...
        def fn1(tmp1):
            ...
            mmax  = tf.cast(tf.math.reduce_max(tf.math.abs(m1),axis=0),tf.complex64)
            # print('mshape: ' + str(mmax.shape))

            def fn2(tmp2):
                c2,m2,f2,r2 = tmp2
                Aobj = Aclass(c2, mmax, f2, fmt, timeline, bool_pol, lam3)
                y2 = myCG(Aobj, r2)
                return y2
    ...
    def call(self, x):
        ...
        def backward1(tmp1):
            ...
            def backward2(tmp2):
                ...
                fmt_sub     =   fmt * tf.expand_dims(mmax, axis=-1) * fm_exp
                # At
                ks          =   k2 * tf.expand_dims(mmax,axis=-1)
                ci          =   tf.linalg.matmul(fmt_sub/220,ks,adjoint_a=True)
...
def create_recon(nx, ny, nc, nLayers, num_block, virtual_coil, num_filters = 64,  esp = 0):
    ...
    dc1         =   c2r(Atb1)
    dc2         =   c2r(Atb2)
   # dc1         = UpdateDC([c2r(Atb1), input_c, input_m1, input_field, fftmtx, timeline, tf.constant(True)])
   # dc2         = UpdateDC([c2r(Atb2), input_c, input_m2, input_field, fftmtx, timeline, tf.constant(False)])
    # define networks
    ...
    #         # get rid of vc
        ...
        # rg1         = UpdateDC.lam_weight([0,krg1])
        # rg2         = UpdateDC.lam_weight([0,krg2])
        # ----ask----
    ...
    return Model(inputs     =   [ input_c, input_m1, input_m2, input_k1, input_k2, input_field],
                 outputs    =   [ out1, out2,r2c(irg1),r2c(irg2),r2c(dc1),r2c(dc2),r2c(rg1),r2c(rg2),Atb2,intermediate_results],
                 name       =   'RECON' )
